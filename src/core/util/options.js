...
export function mergeDataOrFn (
  ...
  if (!vm) {
    ...
    // vm 存在，即生成组件实例的`$options`时，注意这里返回的是函数
...
/**
 * 验证组件的名称是否合法，符合以下几个条件即为合法：
 *   - 组件名称必须以字符开头，能且只能包含任意个这些字符：[A-Za-z0-9_-]
 *   - 不能是 Vue 内置的标签如 slot 和 component
 *   - 不能是平台保留标签，对于 Web 平台来说，保留标签是指 HTML 标签和 SVG 标签
 * @param {*} name 组件名称
 */
