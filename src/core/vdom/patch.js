...
export function createPatchFunction (backend) {
  ...
  /**
   * 判断 vnode 是否是可 patch 的
   */
  function isPatchable (vnode) {
    while (vnode.componentInstance) {
      vnode = vnode.componentInstance._vnode
    }
    // 经过 while 循环后，vnode 是除组件占位 Vnode 之外的第一个 DOM 类型的 vnode（比如元素 vnode、注释 vnode）
    return isDef(vnode.tag)
  }

  /**
   * 往 VNode 对应的 DOM 节点上添加各种模块，包含事件处理函数、class、style 等等
   * @param {*} vnode
   * @param {*} insertedVnodeQueue
   */
  function invokeCreateHooks (vnode, insertedVnodeQueue) {
    for (let i = 0; i < cbs.create.length; ++i) {
      // 调用元素的 create 钩子，详见：https://blog.windstone.cc/vue/source-study/vdom/patch-modules/
      // 包括
      // - 注册 ref
      // - 注册 directives
      // - 添加 class 特性
    ...
    if (isDef(i)) {
      // 调用组件的 create、insert 钩子，详见 https://blog.windstone.cc/vue/source-study/vdom/vnode-tree-create.html#%E5%AE%89%E8%A3%85%E7%BB%84%E4%BB%B6%E7%AE%A1%E7%90%86%E9%92%A9%E5%AD%90%E6%96%B9%E6%B3%95
  ...
  /**
   * 判断 DOM 节点与 VNode 节点是否匹配，主要是判断标签名是否一致
   * @param {*} node DOM 节点
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        // destroy old node
        // 移除、销毁旧节点
        // 这里尤其需要注意，针对根节点 #app 来说，是先将新的根节点插入到 #app 之后，再在此处将 #app 移除掉的。
        // 因此在新根节点插入到文档之后，旧根节点被移除之前，会同时存在两个根节点
        if (isDef(parentElm)) {
